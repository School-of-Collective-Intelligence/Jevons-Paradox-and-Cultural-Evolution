...
def agent_type(m1, m2, m3, m4, eps, t, bs, rebound):
    ...
    for i in range(t):
        if i <= 1:
            ...
            # substitution effect (i.e. agents may start consuming resources in another resource domain)
        if x < 1:
                ...
                se = 0.25
                resources_consumed_by_agent_dom_1 = bs + (x * rebound * (1 - se)) - (1 - rebound) * x
                resources_consumed_by_agent_dom_1 = np.clip(resources_consumed_by_agent_dom_1, 0, None)
                resources_consumed_by_agent_dom_2 = bs + (x * rebound * (se)) - (1 - rebound) * x
                resources_consumed_by_agent_dom_2 = np.clip(resources_consumed_by_agent_dom_2, 0, None)
                resources_consumed_by_agent_dom_3 = bs
                resources_consumed_by_agent_dom_3 = np.clip(resources_consumed_by_agent_dom_3, 0, None)
                ...
                resources_consumed_by_agent_dom_1 = bs + (x * rebound * (1 - se)) - (1 - rebound) * x
                resources_consumed_by_agent_dom_1 = np.clip(resources_consumed_by_agent_dom_1, 0, None)
                resources_consumed_by_agent_dom_2 = bs + (x * rebound * (se) * (1 - se2)) - (1 - rebound) * x
                resources_consumed_by_agent_dom_2 = np.clip(resources_consumed_by_agent_dom_2, 0, None)
                resources_consumed_by_agent_dom_3 = bs + (x * rebound * (se) * (se2))
                resources_consumed_by_agent_dom_3 = np.clip(resources_consumed_by_agent_dom_3, 0, None)
